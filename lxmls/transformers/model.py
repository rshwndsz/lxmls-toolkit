...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        query = query.view(
            B, T, self.num_heads, self.hidden_size // self.num_heads
        ).transpose(1, 2)
        key = key.view(
            B, T, self.num_heads, self.hidden_size // self.num_heads
        ).transpose(1, 2)
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        keys = [
            k for k in keys if not re.match(r"transformer\.h\.\d+\.attn\.bias", k)
        ]  # ignore these
        sd_keys = [
    ...
    def prompt(self, p_text="", tokens=20, num_samples=1, do_sample=True):
        ...
        if p_text == "":
            ...
            x = torch.tensor(
                [[self.tok.encoder.encoder["<|endoftext|>"]]], dtype=torch.long
            )
        else:
